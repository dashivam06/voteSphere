        ...
        <div class="flex items-center space-x-4">
            ...
            <div class="relative">
                <button class="flex items-center space-x-2 focus:outline-none"
                    ...
                    <p style="display:none;">Role: ${sessionScope.authenticated_user.roles}</p>

                    <c:choose>
                        <c:when test="${fn:toLowerCase(sessionScope.authenticated_user.roles) == 'voter'}">

                        <div class="w-10 h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                                <img src="${sessionScope.authenticated_user.profileImage}"
                                     alt="Profile Image"
                        ...
                        <c:otherwise>
                            <div class="w-8 h-8 rounded-full bg-primary-500 flex items-center justify-center text-white text-sm font-medium">
                                    ${fn:substring(sessionScope.authenticated_user.fname, 0, 1)}${fn:substring(sessionScope.authenticated_user.lname, 0, 1)}
                            </div>
                        </c:otherwise>
                    </c:choose>

                    <div class="hidden md:block text-left">
                        <p class="text-sm font-medium text-gray-800 font-['Poppins']">${sessionScope.authenticated_user.fname} ${sessionScope.authenticated_user.lname}</p>
                        <p class="text-xs text-gray-500 font-['Poppins']">${sessionScope.authenticated_user.email}</p>
                    </div>
                    <div class="hidden md:block text-left">
